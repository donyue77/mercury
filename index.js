...
async function sendLineNotify(userIdOrPhone, name, message) {
  if (!userIdOrPhone || userIdOrPhone === '—') return;
  const isPhone = /^09\\d{8}$/.test(userIdOrPhone);
  try {
    await fetch(BACKEND_URL + '/api/line-notify', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        userId: isPhone ? null : userIdOrPhone,
        phone: isPhone ? userIdOrPhone : null,
        name, message
      })
    });
